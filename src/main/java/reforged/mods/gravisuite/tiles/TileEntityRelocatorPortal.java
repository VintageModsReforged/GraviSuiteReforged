...
public class TileEntityRelocatorPortal extends TileEntity {
    ...
    public int getCoolDownTime(Entity entity) {
        if (!this.entityList.isEmpty()) {
            for (EntityInfo entityInfo : this.entityList) {
                String id = entity instanceof EntityPlayer ? ((EntityPlayer) entity).username : entity.getPersistentID().toString();
                if (entityInfo.entityID.equalsIgnoreCase(id))
    ...
    public void addEntityToList(Entity entity) {
        ...
        if (!this.entityList.isEmpty()) {
            ...
            for (EntityInfo point : this.entityList) {
                ...
                if (point.entityID.equalsIgnoreCase(id)) {
    ...
    public void teleportEntity(Entity paramEntity) {
        if (this.parentTeleportPoint != null) {
            ...
            if (!this.entityList.isEmpty()) {
                String id = paramEntity instanceof EntityPlayer ? ((EntityPlayer) paramEntity).username : paramEntity.getPersistentID().toString();
                for (EntityInfo entityInfo : this.entityList) {
                    if (entityInfo.entityID.equalsIgnoreCase(id))
                        d = entityInfo.yaw;
                }
